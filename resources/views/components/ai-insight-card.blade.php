...
<div x-data="{
    ...
    <template x-if="insight">
        <div class="flex items-center justify-between mb-6 w-full border-b border-gray-50 pb-6">
            <div class="flex items-center gap-5">
                <div
                    class="w-14 h-14 rounded-2xl bg-[#2271b1] flex items-center justify-center text-white shadow-xl shadow-zinc-300/40">
    ...
    <div x-show="loading" class="flex flex-col items-center justify-center py-20 w-full">
        <div class="relative w-20 h-20 mb-8">
            ...
            <div class="absolute inset-0 flex items-center justify-center">
                <i class="fa fa-brain text-4xl text-[#2271b1]"></i>
            </div>
    ...
    <div x-show="!insight && !loading && !error"
        ...
        <div
            ...
            <i class="fa fa-sparkles text-zinc-500 text-4xl"></i>
        </div>
        ...
        <button @click="generate()"
            class="inline-flex items-center gap-3 px-8 py-4 bg-[#2271b1] text-white font-black rounded-2xl hover:bg-[#135e96] transition-all shadow-lg shadow-zinc-200/50 hover:-translate-y-1 active:translate-y-0">
        ...
        <div class="space-y-6 w-full text-left">
            ...
            <div class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100/50">
                <div class="flex items-center justify-between mb-4">
                    <span class="text-xs font-bold text-zinc-500 tracking-tight">{{ __('Overall Voter Tone') }}</span>
                </div>
                <div
                    ...
                    <div :style="`width: ${insight.sentiment_breakdown.Positive}%`"
                        class="bg-[#4a7c59] h-full rounded-l-full transition-all duration-1000"
                        :title="@js(__('Positive'))">
                    </div>
                    <div :style="`width: ${insight.sentiment_breakdown.Neutral}%`"
                        class="bg-[#b8954a] h-full transition-all duration-1000" :title="@js(__('Neutral'))"></div>
                    <div :style="`width: ${insight.sentiment_breakdown.Negative}%`"
                        class="bg-[#b05a5a] h-full rounded-r-full transition-all duration-1000"
                        :title="@js(__('Negative'))"></div>
                </div>
                <div class="flex flex-wrap justify-between mt-4 gap-4">
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-[#f0f5f2] border border-[#c8dbd0] rounded-lg">
                        <span class="w-2.5 h-2.5 rounded-full bg-[#4a7c59]"></span>
                        <span class="text-xs font-bold text-[#3a6347] tracking-tight">{{ __('Positive') }}</span>
                        <span class="text-sm font-black text-[#2a4a35]"
                            x-text="insight.sentiment_breakdown.Positive + '%'"></span>
                    </div>
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-[#faf4e8] border border-[#dfc98a] rounded-lg">
                        <span class="w-2.5 h-2.5 rounded-full bg-[#b8954a]"></span>
                        <span class="text-xs font-bold text-[#7a6030] tracking-tight">{{ __('Neutral') }}</span>
                        <span class="text-sm font-black text-[#5a4020]"
                            x-text="insight.sentiment_breakdown.Neutral + '%'"></span>
                    </div>
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-[#faf0f0] border border-[#deb0b0] rounded-lg">
                        <span class="w-2.5 h-2.5 rounded-full bg-[#b05a5a]"></span>
                        <span class="text-xs font-bold text-[#7a3a3a] tracking-tight">{{ __('Negative') }}</span>
                        <span class="text-sm font-black text-[#5a2020]"
            ...
            <div class="grid md:grid-cols-2 gap-10 relative"
                ...
                <div class="flex flex-col" :class="insight.is_truncated ? 'opacity-40 grayscale-[0.5] blur-[1px]' : ''">
                    <div class="flex items-center gap-2 mb-4">
                        <span class="w-1.5 h-4 bg-[#2271b1] rounded-full"></span>
                        <span
                            class="text-xs font-bold text-zinc-500 tracking-tight">{{ __('Key Recurring Themes') }}</span>
                    </div>
                    <div class="grid gap-3">
                        <template x-for="theme in insight.key_themes" :key="theme.theme">
                            <div
                                class="p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:border-zinc-200 transition-colors group">
                    ...
                    <div class="space-y-3">
                        <template x-for="quote in insight.representative_quotes" :key="quote">
                            <div
                                ...
                                <i class="fa fa-quote-left text-zinc-400 text-xs mt-1 shrink-0"></i>
                    ...
                    <div
                        ...
                        <div
                            ...
                            <a href="{{ route('subscriptions.index') }}"
                                class="inline-flex items-center justify-center w-full px-8 py-4 bg-[#2271b1] text-white font-black rounded-2xl hover:bg-[#135e96] transition-all shadow-lg shadow-zinc-200/50 hover:scale-[1.02] active:scale-[0.98]">
