...
@section('content')
    <div class="mb-8 flex items-center justify-between px-4 sm:px-0">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500 font-medium">Generate and view analytical reports for your research
                surveys.</p>
        </div>
    </div>
        ...
        <form action="{{ url()->current() }}" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div class="relative w-full md:w-64 group">
                ...
                <select name="category"
                    ...
                    <option value="feasibility" {{ request('category') === 'feasibility' ? 'selected' : '' }}>Feasibility
                    </option>
                    <option value="market_research" {{ request('category') === 'market_research' ? 'selected' : '' }}>Market
                        Research</option>
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                <td class="px-8 py-6">
                                    <div class="group-hover:translate-x-1 transition-transform">
                                        <span
                                            class="text-sm font-black text-gray-900 uppercase tracking-tight block mb-0.5">{{ $survey->title }}</span>
                                        <span
                                            class="text-[10px] text-gray-400 font-bold uppercase italic">{{ $survey->category->value ?? 'General' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-6">
