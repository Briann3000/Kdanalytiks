...
<div class="w-full">
    ...
    <header class="sticky top-0 z-50 bg-gray-100 border-b border-gray-100 flex items-center justify-between px-10 py-5">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('surveys.export_pdf', $survey) }}" class="flex items-center gap-2 px-4 py-2 bg-white text-gray-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-gray-200 hover:border-red-600 hover:text-red-600 transition-all shadow-sm">
        ...
        <div x-show="$store.workspace.activeTab === 'overview'" x-transition:enter="transition duration-200" class="space-y-8">
            <div class="flex justify-between items-start">
                <h2 class="text-2xl font-black text-gray-900 tracking-tight">Executive Dashboard</h2>
                <span class="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-indigo-100">Live View</span>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Total Responses</p>
                    </div>
                </div>
                <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Analyzed Fields</p>
                    </div>
                </div>
                <div class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none">Start Date</p>
                    </div>
                </div>
            </div>
                ...
                <div class="relative z-10">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-200 font-bold opacity-80 uppercase tracking-tighter">Automatic Strategic Insights</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        @forelse($validLines as $line)
                            ...
                            <div class="flex gap-4 items-start p-4 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                                <i class="fa-solid fa-bolt text-xs mt-1.5 text-indigo-300"></i>
                                <p class="text-sm font-medium leading-relaxed">{{ $trimmed }}</p>
                            </div>
                        @empty
                            <p class="text-indigo-200 italic font-medium">Processing survey intelligence...</p>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        <!-- Quantitative Tab -->
        <div x-show="$store.workspace.activeTab === 'quantitative'" 
             x-data="chartManager()"
            ...
            @foreach($analysis as $item)
                <div class="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm" id="q-{{ $item['id'] }}">
                    <div class="flex justify-between items-start mb-6 border-b border-gray-50 pb-4">
                        <h4 class="text-xl font-black text-gray-900"><span class="text-indigo-600 mr-2">#{{ $loop->iteration }}</span> {{ $item['label'] }}</h4>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black bg-gray-100 text-gray-500 uppercase tracking-widest">
                            {{ $item['type'] }}
                        </span>
                    </div>
                    ...
                    @else
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                ...
                                <div class="flex flex-col gap-4">
                                    ...
                                    <div class="flex items-center justify-between bg-gray-50 p-2 rounded-xl border border-gray-100">
                                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Visualization Type</span>
                                        <div class="flex flex-wrap gap-1">
                                            @foreach(['bar' => 'Bar', 'pie' => 'Pie', 'doughnut' => 'Donut', 'line' => 'Line', 'polarArea' => 'Area', 'radar' => 'Radar'] as $type => $label)
                                                <button @click="switchChartType('{{ $item['canvasId'] }}', '{{ $type }}')" 
                                                        ...
                                                        :class="chartTypes['{{ $item['canvasId'] }}'] === '{{ $type }}' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:bg-white hover:shadow-sm'">
                                                    {{ $label }}
                                                </button>
                                            @endforeach
                                        </div>
                                    ...
                                    <!-- Canvas Container -->
                                    <div class="relative h-72 lg:h-80 w-full flex items-center justify-center bg-gray-50/30 rounded-2xl p-6 border border-gray-50">
                                ...
                                <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                                    <div class="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                                        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Frequency Distribution</span>
                                        <span class="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded uppercase">n={{ $item['answered_count'] }}</span>
                                    </div>
                                        ...
                                        <table class="w-full text-left text-sm">
                                            ...
                                            <tbody class="divide-y divide-gray-50">
                                                @foreach($item['stats'] as $stat)
                                                    <tr class="{{ isset($stat['is_missing']) ? 'bg-orange-50/30' : 'hover:bg-gray-50' }} transition-colors">
                                                        <td class="px-4 py-3 font-medium {{ isset($stat['is_missing']) ? 'text-orange-600 italic' : 'text-gray-700' }}">
                                                            {{ $stat['value'] }}
                                                        </td>
                                                        <td class="px-4 py-3 text-right font-black text-gray-900">{{ $stat['count'] }}</td>
                                                        <td class="px-4 py-3 text-right">
                                                            <div class="flex items-center justify-end gap-2">
                                                                <span class="font-bold text-indigo-600">{{ $stat['percentage'] }}%</span>
                                                                <div class="w-12 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                                                                    <div class="bg-indigo-500 h-full" style="width: {{ $stat['percentage'] }}%"></div>
                                                                </div>
                                                            </div>
                                                        </td>
                                                    </tr>
                        ...
                        @else
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                <div class="bg-indigo-50/30 rounded-2xl p-8 border border-indigo-100 flex flex-col justify-center text-center">
                                    <i class="fa-solid fa-brain text-indigo-300 text-4xl mb-4"></i>
                                    <h5 class="text-lg font-black text-indigo-900 mb-2">Qualitative Data</h5>
                                    <p class="text-indigo-600 text-[11px] font-bold uppercase tracking-wider mb-6">Open-ended responses feed</p>
                                    <button @click="$store.workspace.setTab('qualitative'); $store.workspace.scrollTo('ql-{{ $item['id'] }}')" class="mx-auto px-6 py-2 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                                        View Deep Insights
                                    </button>
                                </div>

                                <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                                    <div class="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                        Latest Responses
                                    </div>
                                    <div class="max-h-64 overflow-y-auto custom-scrollbar p-1">
                                        @forelse(array_slice($item['answers'] ?? [], 0, 5) as $ans)
                                            <div class="p-4 text-xs font-medium text-gray-600 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                                                "{{ str($ans)->limit(150) }}"
                                            </div>
                                        @empty
        ...
        <!-- Qualitative Tab -->
        <div x-show="$store.workspace.activeTab === 'qualitative'" x-transition:enter="transition duration-200" style="display: none;" class="space-y-12">
                ...
                @if(!$item['isChartable'])
                    <div class="bg-white rounded-3xl p-10 shadow-sm border border-gray-100" id="ql-{{ $item['id'] }}">
                        <div class="mb-8 border-b border-gray-50 pb-6">
                            <span class="text-[10px] font-black text-indigo-500 uppercase tracking-widest block mb-2">Question Analysis</span>
                            <h4 class="text-2xl font-black text-gray-900 tracking-tight">
                                <span class="text-indigo-600 mr-2">Q{{ $loop->iteration }}.</span> {{ $item['label'] }}
                            </h4>
                        </div>
                        ...
                        <div class="mt-12 bg-gray-50 rounded-2xl p-6 border border-gray-100">
                            <h5 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-6">Individual Responses Feed</h5>
                            <div class="max-h-[400px] overflow-y-auto pr-2 custom-scrollbar space-y-3">
                                @forelse($item['answers'] as $answer)
                                    <div class="bg-white p-5 rounded-xl border border-gray-100 shadow-sm text-gray-700 text-sm leading-relaxed relative group overflow-hidden">
        ...
        <!-- Tables Tab -->
        <div x-show="$store.workspace.activeTab === 'tables'" x-transition:enter="transition duration-200" style="display: none;" class="space-y-8">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-black text-gray-900 tracking-tight">Data Explorer</h2>
                <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1 rounded-lg border border-gray-100">
                    Excel/SPSS Format
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-max">
                        <thead>
                            <tr class="bg-gray-900 text-white">
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] border-r border-gray-800 sticky left-0 bg-gray-900 z-10">ID</th>
                                <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] border-r border-gray-800">Date Submitted</th>
                                @foreach($analysis as $item)
                                    <th class="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] border-r border-gray-800 last:border-r-0 min-w-[200px]" title="{{ $item['label'] }}">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($responses as $response)
                                <tr class="hover:bg-indigo-50/30 transition-colors group">
                                    <td class="px-6 py-4 font-black text-indigo-600 text-sm border-r border-gray-50 sticky left-0 bg-white group-hover:bg-indigo-50 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                        #{{ $response->id }}
                                    </td>
                                    <td class="px-6 py-4 text-xs font-bold text-gray-500 border-r border-gray-50 uppercase tracking-tighter">
                                    ...
                                    @endphp
                                    @foreach($analysis as $item)
                                        <td class="px-6 py-4 text-sm text-gray-700 border-r border-gray-50 last:border-r-0 italic font-medium">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ count($analysis) + 2 }}" class="py-20 text-center text-gray-400 font-bold uppercase tracking-widest text-xs">
            ...
            <div class="flex justify-center pt-4">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Viewing latest {{ count($responses) }} responsive blocks</p>
            </div>
        </div>

        <!-- Gallery Tab -->
        <div x-show="$store.workspace.activeTab === 'gallery'" x-transition:enter="transition duration-200" style="display: none;">
            <div class="h-full flex flex-col items-center justify-center py-24 text-center">
                <div class="w-32 h-32 bg-emerald-50 rounded-[3rem] flex items-center justify-center mb-10 transform rotate-3">
                ...
                <p class="text-lg text-gray-500 font-medium max-w-sm mx-auto leading-relaxed">
                    Aggregate and view all uploaded photoblocks, voice recordings, and videos in this gallery workspace (Phase 4).
                </p>
            </div>
        </div>
