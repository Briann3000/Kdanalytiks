...
<div x-data="{ billingCycle: 'monthly' }" class="py-12 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-16">
            <h2 class="text-base font-black text-[#2271b1] tracking-widest uppercase mb-4">{{ __('Pricing Plans') }}</h2>
            ...
            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-8 mb-4">{{ __('Account Type') }}: {{ __('Researcher') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
            ...
            @foreach($tiers as $tier)
                ...
                @endphp
                <div class="relative flex flex-col bg-white  shadow-xl transition-all duration-500 hover:scale-[1.03] hover:shadow-2xl overflow-hidden {{ $isPro ? 'border-4  z-10' : 'border border-gray-100' }}">
                    @if($isPro)
                        <div class="absolute top-0 right-0 bg-zinc-100 text-blue-800 px-6 py-2 font-black text-[12px]tracking-widest shadow-lg">{{ __('Most Popular') }}</div>
                    @endif
                    ...
                    <div class="p-10 flex-1 flex flex-col">
                        <div class="mb-8">
                            ...
                            <div class="flex items-baseline gap-1">
                                ...
                                <span class="text-gray-400 font-bold text-[10px] tracking-widest" x-text="billingCycle === 'monthly' ? '{{ __('/ Month') }}' : '{{ __('/ Year') }}'"></span>
                            </div>
                        </div>
                        ...
                        <ul class="space-y-5 mb-10 flex-1">
                            @if(isset($accountTypeLabel) && str_contains($accountTypeLabel, 'Respondent'))
                                <li class="flex items-start gap-3">
                                    <div class="mt-1 flex-shrink-0 w-5 h-5 bg-zinc-100 rounded-full flex items-center justify-center">
                        ...
                        <div class="mt-auto">
                            <form action="{{ route('subscriptions.checkout') }}" method="POST">
                                ...
                                @if($isCurrent)
                                    <button type="button" disabled class="w-full bg-zinc-100 text-[#2271b1] font-black py-5 rounded-2xl cursor-not-allowed tracking-widest text-sm border border-zinc-200">
                                        {{ __('Current Plan') }}
                                    </button>
                                    @if($tier->slug !== 'free')
                                ...
                                @else
                                    ...
                                    @endphp
                                    <button type="submit" class="w-full {{ $isPro ? 'bg-[#2271b1] text-white shadow-zinc-300/40' : 'bg-[#2271b1] text-white shadow-gray-200' }} hover:scale-[1.02] active:scale-[0.98] transition-all py-5 rounded-2xl shadow-xl tracking-widest {{ $isDowngrade ? 'text-sm font-medium' : 'text-xs font-black' }}">
                                        {{ $btnLabel }}
                                    </button>
                                @endif
                            </form>
        ...
        <div class="mt-20 bg-white rounded-3xl p-10 border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="max-w-xl">
                ...
                <p class="text-gray-500 font-medium">{{ __('Contact our team for a quote if you have unique requirements or need custom integrations.') }}</p>
            </div>
            <a href="mailto:infokdanalytiks@gmail.com" class="px-8 py-4 bg-zinc-100 text-[#135e96] font-black rounded-2xl hover:bg-zinc-200 transition-colors uppercase tracking-widest text-xs">
                {{ __('Contact Sales') }}
            </a>
        </div>
    </div>
