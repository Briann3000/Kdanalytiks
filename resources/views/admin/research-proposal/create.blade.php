...
@section('title', __('Draft Research Proposal — KDAnalytiks'))

@section('content')
    @include('admin.research-proposal.wizard')
@endsection

{{-- LEGACY PROPOSAL FORM [ROLLBACK SAFE]
@extends('layouts.app')
        ...
        <form action="{{ route('research-proposal.store') }}" method="POST" class="space-y-8" id="proposalForm">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 gap-6 border-b border-gray-50 pb-8">
                    <div>
                        <label for="title" class="block text-xs font-black text-gray-500 tracking-wider mb-2">{{
                            __('Proposal Title') }}</label>
                    ...
                    <div>
                        <label for="research_question"
                            class="block text-xs font-black text-gray-500 tracking-wider mb-2">{{ __('Primary Research
                            Question & Main Objectives') }}</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-gray-50 pb-8">
                    <div>
                        <label for="objectives" class="block text-xs font-black text-gray-500 tracking-wider mb-2">{{
                            __('Specific Objectives') }}</label>
                    ...
                    <div>
                        <label for="scope" class="block text-xs font-black text-gray-500 tracking-wider mb-2">{{
                            __('Target Population & Geographic Scope') }}</label>
                ...
                <div
                    class="border-b border-gray-50 pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <label for="style" class="block text-xs font-black text-gray-500 tracking-wider mb-1">{{
                            __('Academic Citation Standard') }}</label>
                        <p class="text-[10px] text-gray-400 font-medium">{{ __('Select standard for in-text citations
                            and reference list formatting.') }}</p>
                    </div>
                ...
                <div class="border-b border-gray-50 pb-8 space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h4 class="text-xs font-black text-gray-500 tracking-wider uppercase">{{ __('Proposed Budget
                                (Optional)') }}</h4>
                            <p class="text-[10px] text-gray-400 font-medium">{{ __('Included in proposal document
                                exports. If left empty, budget section is cleanly excluded.') }}</p>
                        </div>
                    ...
                    <div class="space-y-3">
                        <template x-for="(item, index) in budgetItems" :key="index">
                            <div class="flex items-center space-x-3">
                                <input type="text" :name="'budget['+index+'][item]'" x-model="item.item"
                                    placeholder="{{ __('Budget item description (e.g. Fieldwork enumerators compensation)') }}"
                                    class="flex-1 bg-gray-50 border-gray-100 rounded-2xl px-4 py-3 text-xs font-medium focus:ring-2 focus:ring-[#2271b1]">
                                <div class="relative w-44">
                                    <span
                                        class="absolute inset-y-0 left-0 pl-3 flex items-center text-xs font-bold text-gray-400">KES</span>
                                    <input type="number" :name="'budget['+index+'][cost]'" x-model="item.cost"
                                        placeholder="0.00"
                                        class="w-full bg-gray-50 border-gray-100 rounded-2xl pl-12 pr-4 py-3 text-xs font-bold focus:ring-2 focus:ring-[#2271b1]">
                                </div>
                                <button type="button" @click="budgetItems.splice(index, 1)"
                                    class="p-3 text-gray-300 hover:text-red-500 transition-colors">
                ...
                <div
                    class="p-6 bg-gradient-to-r from-blue-50/80 via-indigo-50/50 to-white rounded-3xl border-2 border-blue-200/80 shadow-xs space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div
                                class="w-8 h-8 rounded-xl bg-[#2271b1] text-white flex items-center justify-center font-bold text-xs shadow-xs">
                                <i class="fa-solid fa-wand-magic-sparkles"></i>
                            </div>
                            <div>
                                <h4 class="text-xs font-black text-gray-900 tracking-wider uppercase">{{ __('Custom
                                    Formatting & Voice Presets / KB Guidelines') }}</h4>
                                <p class="text-[10px] text-gray-500 font-medium">{{ __('Specify tone, structure,
                                    formatting preferences, or things to omit/add before generating.') }}</p>
                            </div>
                        </div>
                        <span
                            class="px-2.5 py-1 text-[10px] font-black bg-blue-100 text-[#2271b1] rounded-lg tracking-wider">{{
                            __('PRO FEATURE') }}</span>
                    </div>
            ...
            <div class="flex items-center justify-between pt-2">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('research-proposal.history') }}"
                        class="px-8 py-4 rounded-2xl text-xs font-black text-gray-500 tracking-widest hover:bg-gray-100 transition-all">{{
                        __('Cancel') }}</a>
