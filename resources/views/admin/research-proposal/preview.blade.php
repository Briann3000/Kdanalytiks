        ...
        <div class="flex items-center space-x-3">
            ...
            <form action="{{ route('research-proposal.generate') }}" method="POST" class="flex items-center space-x-2" id="regenerateForm">
                ...
                <select name="style" onchange="this.form.submit(); this.disabled=true;" class="bg-indigo-50 text-indigo-600 text-[8px] font-black rounded border border-indigo-100 uppercase py-0.5 px-2 focus:ring-1 focus:ring-indigo-300 cursor-pointer hover:bg-indigo-100 transition-all outline-none">
                    ...
                    <option value="ieee" {{ ($reportData['metadata']['style'] ?? '') === 'ieee' ? 'selected' : '' }}>IEEE</option>
                    <option value="vancouver" {{ ($reportData['metadata']['style'] ?? '') === 'vancouver' ? 'selected' : '' }}>Vancouver</option>
                    <option value="oscola" {{ ($reportData['metadata']['style'] ?? '') === 'oscola' ? 'selected' : '' }}>OSCOLA</option>
                </select>
            </form>
        </div>
                ...
                @foreach($reportData['sections'] as $title => $content)
                    ...
                    @elseif(str_contains($content, "<div class='data-table'") || str_contains($content, "<table"))
                        ...
                        <section id="section-{{ $loop->iteration }}" class="scroll-mt-32 relative">
                            ...
                            @else
                                <h3 class="text-lg font-black text-gray-900 mb-6 border-l-4 border-indigo-600 pl-5 tracking-tight leading-snug">{{ $cleanContentTitle }}</h3>
                            @endif
                    ...
                    @else
                        ...
                        <section id="section-{{ $loop->iteration }}" class="scroll-mt-32 relative">
                            ...
                            @elseif(in_array($title, ['Declaration', 'Acknowledgement', 'Abstract', 'Abbreviations', 'Definition of Key Terms']))
                                <h3 class="text-lg font-black text-gray-900 mb-6 tracking-tight uppercase leading-none text-center border-b border-gray-100 pb-4">{{ $cleanContentTitle }}</h3>
                            @else
                                <h3 class="text-lg font-black text-gray-900 mb-6 border-l-4 border-indigo-600 pl-5 tracking-tight leading-snug">{{ $cleanContentTitle }}</h3>
                            @endif
                            <div class="prose prose-slate prose-lg max-w-none text-gray-700 leading-relaxed font-serif text-justify whitespace-pre-line">
                                {!! nl2br(e($content)) !!}
                            </div>
                        </section>
                    @endif
...
<style>
    ...
    .prose table { font-family: system-ui, sans-serif; }
