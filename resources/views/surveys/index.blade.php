...
<div x-data="{ 
    ...
    async bulkDelete() {
        ...
        const result = await Swal.fire({
            ...
            html: `You are about to delete <b>${this.selected.length}</b> surveys and all their associated data.<br><br><span class='text-red-500 font-bold uppercase text-[10px] tracking-widest'>This action cannot be undone.</span>`,
            icon: 'warning',
    ...
    <div class="flex items-center justify-between mb-8 px-4 sm:px-0">
        ...
        <div>
            <a href="{{ route('surveys.create') }}" class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">
                <i class="fa-solid fa-plus mr-2"></i> New Survey
            </a>
        </div>
        @endif
    </div>
    ...
    <div x-show="selected.length > 0" 
        ...
        <div class="flex items-center text-white">
            ...
            <span class="text-xs font-black uppercase tracking-widest">Surveys Selected</span>
        </div>
            ...
            <table class="min-w-[900px] w-full divide-y divide-gray-100">
                <thead class="bg-gray-50/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-5 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Access</th>
                                    ...
                                    @if($survey->is_paid)
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-emerald-500 text-white shadow-sm ring-1 ring-emerald-600/20">
                                                <i class="fa-solid fa-sack-dollar mr-1.5 leading-none"></i> Paid ({{ number_format((float)$rewardVal, 0) }} KES)
                                            </span>
                                        @endif
                                    @endif
                                </div>
