    ...
    <div class="w-full flex flex-col items-center py-12 px-4">
        <div class="max-w-5xl w-full">
            ...
            <form action="{{ route('surveys.initialize') }}" method="POST" id="categoryForm">
                ...
                <div class="mb-8">
                    <label for="title"
                        class="block text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1">Survey
                        Name</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @foreach($categories as $key => $cat)
                        <label class="relative group cursor-pointer">
                            ...
                            <div
                                class="h-full bg-white border-2 border-gray-100 rounded-3xl p-6 transition-all duration-300 hover:border-[#2271b1] hover:shadow-xl hover:shadow-sm0/5 peer-checked:border-[#2271b1] peer-checked:bg-zinc-100 peer-checked:ring-4 peer-checked:ring-[#2271b1]/5 flex flex-col">
                                <div
                                    class="w-14 h-14 rounded-2xl bg-{{ $cat['color'] }}-50 flex items-center justify-center text-{{ $cat['color'] }}-600 mb-6 group-hover:scale-110 transition-transform shadow-sm border border-{{ $cat['color'] }}-100">
                                    <i class="fa-solid {{ $cat['icon'] }} text-2xl"></i>
                                </div>
                                <h3 class="text-sm font-black text-gray-900 uppercase tracking-widest mb-2">{{ $cat['label'] }}
                                </h3>
                                <p class="text-xs text-gray-400 font-bold leading-relaxed line-clamp-3 italic">
                                    {{ $cat['desc'] }}
                                </p>

                                <div
                                    class="mt-auto pt-6 flex justify-end opacity-0 group-hover:opacity-100 peer-checked:opacity-100 transition-opacity">
                                    <div
                                        class="w-8 h-8 rounded-full bg-[#2271b1] text-white flex items-center justify-center shadow-lg shadow-zinc-300/40">
                ...
                <div class="mt-12 flex justify-center">
                    <button type="submit"
                        class="px-12 py-5 bg-[#2271b1] text-white rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-zinc-300/40 hover:bg-[#135e96] hover:-translate-y-1 transition-all flex items-center group">
