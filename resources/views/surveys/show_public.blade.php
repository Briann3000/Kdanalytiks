...
@section('meta_description', \Illuminate\Support\Str::limit(strip_tags($survey->description ?? 'Participate in this survey on KDAnalytiks.'), 150))
...
@section('head')
    ...
    <style>
        ...
        /* Header & Typography Hierarchy Overrides */
        .rendered-form h1,
        .rendered-form .field-header h1,
        .rendered-form h1.form-group-header,
        .rendered-form [data-type="header"] h1 {
        ...
        .rendered-form h2,
        .rendered-form .field-header h2,
        .rendered-form h2.form-group-header,
        .rendered-form [data-type="header"] h2 {
        ...
        .rendered-form h3,
        .rendered-form .field-header h3,
        .rendered-form h3.form-group-header,
        .rendered-form [data-type="header"] h3 {
        ...
        .rendered-form p,
        .rendered-form .field-paragraph,
        .rendered-form .field-note,
        .rendered-form .field-description {
...
@section('content')
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-100">
            <!-- Header Section -->
            <div class="px-8 py-8 bg-gradient-to-r from-gray-900 via-zinc-800 to-zinc-900 relative">
                <div class="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-white/10 rounded-full blur-2xl"></div>
                <div class="absolute bottom-0 left-0 -mb-10 -ml-10 w-32 h-32 bg-zinc-1000/20 rounded-full blur-xl"></div>

                <div class="relative z-10 flex justify-between items-start">
                    <div>
                        <h1 class="text-3xl font-black text-white tracking-tight">{{ $survey->title }}</h1>
                        <p class="mt-3 text-zinc-300/90 font-medium leading-relaxed max-w-2xl">{{ $survey->description }}
                        </p>
                        ...
                        @elseif($survey->type === \App\Enums\SurveyType::Public)
                            <div
                                ...
                                <span
                                    class="text-[10px] font-black uppercase tracking-widest leading-none">{{ __("Public Survey") }}</span>
                            </div>
                        @endif
                    </div>
                ...
                <div class="bg-green-50 border-l-4 border-green-500 p-6 m-8 rounded-r-xl">
                    <div class="flex">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-sm text-green-600 mt-1">Your responses have been recorded.</p>
                        </div>
                    </div>
                </div>
                <script>
            ...
            @else
                @if(Auth::guest())
                    <div id="guest-details-overlay"
                        class="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-gray-200 m-6 animate-fade-in relative shadow-sm max-w-lg mx-auto">
                        <div class="w-20 h-20 bg-zinc-100 rounded-2xl flex items-center justify-center mx-auto mb-6">
                            <i class="fa-solid fa-user text-3xl text-zinc-2000"></i>
                        </div>
                        <h2 class="text-3xl font-black text-gray-900 mb-4 tracking-tight">{{ __('Welcome!') }}</h2>
                        <p class="text-gray-600 mb-6 font-medium">
                            {{ __('Please provide your details below to proceed with the survey.') }}
                        </p>
                        <div class="space-y-4 text-left max-w-sm mx-auto">
                            <div>
                                <label for="temp_guest_name"
                                    class="block text-xs font-black uppercase text-gray-400 tracking-wider mb-2">{{ __('Name') }}
                                    <span class="text-red-500">*</span></label>
                                <input type="text" id="temp_guest_name" required
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl font-bold text-gray-700 focus:ring-2 focus:ring-[#2271b1] focus:border-[#2271b1]">
                            </div>
                            <div>
                                <label for="temp_guest_phone"
                                    class="block text-xs font-black uppercase text-gray-400 tracking-wider mb-2">{{ __('Phone Number (Optional)') }}</label>
                                <input type="tel" id="temp_guest_phone"
                                    class="w-full px-4 py-3 border border-gray-200 rounded-xl font-bold text-gray-700 focus:ring-2 focus:ring-[#2271b1] focus:border-[#2271b1]">
                            </div>
                            <button type="button" onclick="startSurveyAsGuest()"
                                class="w-full mt-2 py-4 bg-[#2271b1] text-white text-[11px] font-black uppercase tracking-widest rounded-xl shadow-lg shadow-zinc-200/50 hover:bg-[#135e96] transition-all transform hover:-translate-y-0.5">
                                {{ __('Start Survey') }}
                            </button>
                        </div>
                    </div>
                ...
                <div id="survey-content-wrapper" class="p-6 sm:p-10 min-h-[500px] {{ Auth::check() ? '' : 'hidden' }}">
                    ...
                    @if(!empty($survey->json_schema) && $survey->json_schema !== '[]')
                        ...
                        <form id="jsonSurveyForm"
                            ...
                            @csrf
                            <input type="hidden" name="guest_name" id="guest_name_json" value="">
                            <input type="hidden" name="guest_phone" id="guest_phone_json" value="">
                            <div id="surveyContainer" class="bg-gray-50/50 p-8 rounded-2xl border border-gray-100 shadow-sm mb-6">
                                <div id="surveyLoading" class="flex flex-col items-center justify-center py-20">
                                    <i class="fa-solid fa-spinner fa-spin text-4xl text-zinc-2000 mb-4"></i>
                                ...
                                <label class="flex items-start cursor-pointer group">
                                    <div class="flex items-center h-6">
                                        <input id="terms_and_conditions" name="terms_and_conditions" type="checkbox" required
                                            class="h-5 w-5 text-[#2271b1] border-gray-300 rounded focus:ring-[#2271b1] transition-all cursor-pointer">
                                    </div>
                                    <div class="ml-4 text-sm">
                                        <span
                                            class="font-bold text-gray-900 text-base group-hover:text-[#135e96] transition-colors">
                                            I agree to the <a href="{{ route('terms') }}" target="_blank"
                                                onclick="event.stopPropagation();"
                                                class="text-[#2271b1] hover:underline font-bold">{{ __("Terms and Conditions") }}</a>
                                        </span>
                                        <p class="text-gray-500 mt-1 leading-relaxed">By submitting this survey, you acknowledge
                                            that your responses will be recorded and processed in accordance with our <a
                                                ...
                                                class="text-[#2271b1] hover:underline font-bold">{{ __("Data Privacy Policy") }}</a>.
                                            We value
                            ...
                            <div id="submitContainer" class="pt-6 border-t border-gray-100 flex justify-end gap-4 hidden">
                                <button type="button" onclick="window.resetSurvey()"
                                    class="inline-flex items-center px-8 py-4 border border-gray-300 text-base font-bold rounded-xl shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2271b1] transition-all">
                                    <i class="fa-solid fa-rotate-left mr-2"></i> Reset Answers
                                </button>
                                <button type="submit"
                                    class="inline-flex items-center px-8 py-4 border border-transparent text-base font-bold rounded-xl shadow-lg text-white bg-gray-900 hover:bg-[#2271b1] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2271b1] transition-all transform hover:-translate-y-1">
                        ...
                        <script>
                            ...
                            window.addRepeatEntry = function (id) {
                                ...
                                entry.innerHTML = `<div class="repeat-entry-header"><span class="text-[10px] font-black text-zinc-2000 uppercase">Entry #${idx}</span><button type="button" onclick="window.removeRepeatEntry('${id}', ${idx})" class="text-red-400 text-xs font-bold">{{ __("Remove") }}</button></div><input type="text" name="${id}_entry_${idx}" class="w-full px-4 py-2 border rounded-xl mt-1" oninput="window.syncRepeatData('${id}')">`;
                            ...
                            window.onload = function () {
                                ...
                                const processedSchema = surveyData.map(field => {
                                    ...
                                    console.log("Original field:", field.name, "Type:", field.type, "Mapped type:", finalType);
                                    ...
                                    if (field.type === 'group') {
                                        console.log("Mapping group field to header:", field.name);
                                        fieldClone.type = 'header';
                                        fieldClone.subtype = 'h3';
                                        fieldClone.className = (fieldClone.className || '') + ' preview-group-header';
                                    ...
                                    console.log("Final mapped field:", fieldClone.name, "Type:", fieldClone.type);
                                ...
                                const renderOptions = {
                                    ...
                                    templates: {
                                            ...
                                            return {
                                                field: `
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-4">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <div id="audio_preview_container_${id}" class="hidden mt-4 p-3 bg-slate-50 border border-slate-200 rounded-xl" style="display:none;">
                                                                                                                                                                                                                                                                                        <label for="player_${id}" class="block mb-2 text-xs font-semibold uppercase tracking-wider text-slate-600">
                                                                                                                                                                                                                                                                                            {{ __('Preview') }}
                                                                                                                                                                                                                                                                                        </label>
                                                                                                                                                                                                                                                                                        <audio id="player_${id}" controls class="w-full rounded-lg shadow-sm"></audio>
                                                                                                                                                                                                                                                                                    </div>
                                        ...
                                        'calculate_display': function (fieldData) {
                                            const id = fieldData.name;
                                            return {
                                                field: `<div class="p-5 bg-gray-50 rounded-2xl border border-gray-100 mb-4"><div class="text-2xl font-black text-[#2271b1]" id="calc_display_${id}">&mdash;</div><input type="hidden" name="${id}" id="input_${id}" value="" data-formula="${fieldData.formula || ''}"></div>`,
                                        ...
                                        'location_picker': function (fieldData) {
                                            const id = fieldData.name;
                                            return {
                                                field: `<div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-4"><div id="map_${id}" class="location-map-container" style="height:250px;border-radius:1rem;background:#e2e8f0;"></div><div class="flex items-center gap-3 mt-3"><button type="button" onclick="window.captureLocation('${id}')" class="px-4 py-2 bg-[#2271b1] text-white rounded-xl text-xs font-bold uppercase hover:bg-[#135e96] transition-all"><i class="fa-solid fa-location-crosshairs mr-2"></i>{{ __("Capture My Location") }}</button><span id="loc_status_${id}" class="text-[10px] font-bold text-gray-400 uppercase"></span></div><input type="hidden" name="${id}" id="input_${id}" value="" ${fieldData.required ? 'required' : ''}></div>`,
                                            ...
                                            return {
                                                field: `<div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-4">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            <div id="qr_result_${id}" class="mt-4" style="display:none;">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                <button type="button" onclick="window.resetQRScanner('${id}')" class="mt-2 text-[10px] font-bold text-zinc-2000 uppercase hover:text-[#135e96]">{{ __("Scan Again") }}</button>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            </div>
                                        ...
                                        'likert_matrix_grid': function (fieldData) {
                                            ...
                                            return {
                                                field: `<div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm mb-4"><div style="overflow-x:auto;"><table class="likert-matrix-table" style="width:100%;"><thead><tr>${hdr}</tr></thead><tbody>${body}</tbody></table></div><input type="hidden" name="${id}" id="input_${id}" value="" data-rows='${JSON.stringify(rows)}'></div>`,
                                        ...
                                        'repeat_container': function (fieldData) {
                                            ...
                                            return {
                                                field: `<div class="p-5 rounded-2xl border-2 border-dashed border-zinc-300 mb-4 bg-zinc-100" id="repeat_${id}"><div class="flex items-center gap-2 mb-3"><i class="fa-solid fa-repeat text-zinc-2000"></i><span class="text-sm font-bold text-[#135e96]">{{ __("Repeating Section") }}</span></div><div id="repeat_entries_${id}" class="space-y-3"></div><button type="button" onclick="window.addRepeatEntry('${id}')" class="mt-3 px-4 py-2 bg-zinc-200 text-[#2271b1] rounded-xl text-xs font-bold uppercase hover:bg-zinc-300 transition-all"><i class="fa-solid fa-plus mr-2"></i>{{ __("Add Entry") }}</button><input type="hidden" name="${id}" id="input_${id}" value="[]" data-min="${minR}" data-max="${maxR}"></div>`,
                                ...
                                function resolvePublicFieldGroup(fieldName) {
                                    let group = $(`.field-${fieldName}`).closest('.form-group');
                                    if (!group.length) {
                                        group = $(`[name="${fieldName}"], [name="${fieldName}[]"]`).closest('.form-group');
                                    }
                                ...
                                function updateVisibility() {
                                    processedSchema.forEach(field => {
                                        const group = resolvePublicFieldGroup(field.name);
                                        ...
                                        if (field.visible_if && field.visible_if.field) {
                                            ...
                                            const triggerField = processedSchema.find(candidate => candidate.name === triggerFieldName);
                                ...
                                setTimeout(() => {
                                    processedSchema.forEach(field => {
                                        if (field.visible_if && field.visible_if.field) {
                                            resolvePublicFieldGroup(field.name).hide();
                                        }
                                    });
                                    updateVisibility();
                                }, 300);
                                ...
                                function setupRecorder(id, type, maxDuration = 60) {
                                    ...
                                    const audioContainer = document.getElementById(`audio_preview_container_${id}`);
                                    ...
                                    retakeBtn.onclick = () => {
                                        ...
                                        delete recorderBlobs[id];
                                        blobInput.value = '';//reset hidden form input
                                        player.src = '';
                                ...
                                if (userData && userData.length > 0) {
                                    ...
                                    if (hasRestored) {
                                        $('#surveyContainer').prepend('<div class="bg-zinc-100 text-[#135e96] p-3 rounded-lg text-sm font-medium mb-4 flex justify-between items-center shadow-sm border border-zinc-200" id="draftToast"><div class="flex items-center"><i class="fa-solid fa-cloud-arrow-down mr-2"></i> Your previous draft has been restored from local storage (Offline Draft Recovery Active).</div><button type="button" onclick="$(\'#draftToast\').fadeOut()" class="text-zinc-2000 hover:text-[#135e96]"><i class="fa-solid fa-xmark"></i></button></div>');
                                    }
                                }
                                ...
                                $('#jsonSurveyForm').on('submit', function (e) {
                                    ...
                                    const performSubmit = () => {
                                        ...
                                        const termsById = document.getElementById('terms_and_conditions');
                                        const termsByName = document.querySelector('input[name="terms_and_conditions"]');
                                        const termsByAnyCheck = document.querySelector('input[type="checkbox"]');

                                        console.log('Terms by ID:', termsById);
                                        console.log('Terms by Name:', termsByName);
                                        console.log('Any Checkbox:', termsByAnyCheck);
                                        console.log('Is Checked?:', termsById ? termsById.checked : 'Element Missing');

                                        const formData = new FormData(document.getElementById('jsonSurveyForm'));
                                        console.log('Terms in FormData:', formData.get('terms_and_conditions'));
                                        ...
                                        const termsCheckbox = document.getElementById('terms_and_conditions')
                                            || document.querySelector('input[name="terms_and_conditions"]');

                                        if (termsCheckbox && termsCheckbox.checked) {
                                            formData.set('terms_and_conditions', '1');
                                        } else {
                                            formData.delete('terms_and_conditions');
                                        }
                                        console.log('Terms AFTER set:', formData.get('terms_and_conditions'));
                    ...
                    @else
                        ...
                        <form id="legacySurveyForm"
                            ...
                            @csrf
                            <input type="hidden" name="guest_name" id="guest_name_legacy" value="">
                            <input type="hidden" name="guest_phone" id="guest_phone_legacy" value="">
                                ...
                                <div
                                    ...
                                    <div class="flex items-start mb-6">
                                        <div class="flex-shrink-0 mr-4 mt-1">
                                            <div
                                                class="w-8 h-8 bg-zinc-200 text-[#2271b1] rounded-full flex items-center justify-center font-bold text-sm">
                                                {{ $index + 1 }}
                                            </div>
                                        </div>
                                        <div class="flex-grow">
                                    ...
                                    <div class="ml-12">
                                        ...
                                        @if ($question->type === 'text')
                                            <input type="text" name="{{ $inputName }}" {{ $question->required ? 'required' : '' }}
                                                class="w-full px-5 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2271b1] focus:border-[#2271b1] transition-all text-gray-700 shadow-sm font-medium">

                                        @elseif ($question->type === 'textarea')
                                            <textarea name="{{ $inputName }}" rows="4" {{ $question->required ? 'required' : '' }}
                                                class="w-full px-5 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2271b1] focus:border-[#2271b1] transition-all text-gray-700 shadow-sm font-medium"></textarea>
                                        ...
                                        @elseif ($question->type === 'radio')
                                            <div class="space-y-3">
                                                ...
                                                @foreach ($options as $option)
                                                    <label
                                                        class="flex items-center p-4 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-zinc-400 hover:bg-zinc-100 transition-all">
                                                        <input type="radio" name="{{ $inputName }}" value="{{ $option }}" {{ $question->required ? 'required' : '' }}
                                                            class="w-5 h-5 text-[#2271b1] border-gray-300 focus:ring-[#2271b1]">
                                                ...
                                                @foreach ($options as $option)
                                                    <label
                                                        ...
                                                        <input type="checkbox" name="{{ $inputName }}[]" value="{{ $option }}"
                                                            class="w-5 h-5 text-[#2271b1] rounded border-gray-300 focus:ring-[#2271b1]">
                                                        <span class="ml-4 text-gray-700 font-medium">{{ $option }}</span>
                                                    </label>
                                                @endforeach
                                            </div>
                                        ...
                                        @elseif ($question->type === 'integer' || $question->type === 'number')
                                            <input type="number" name="{{ $inputName }}" {{ $question->required ? 'required' : '' }}
                                                class="w-full md:w-1/2 px-5 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#2271b1] transition-all text-gray-700 shadow-sm">

                                        @elseif ($question->type === 'email')
                                            <input type="email" name="{{ $inputName }}" {{ $question->required ? 'required' : '' }}
                                                class="w-full px-5 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#2271b1] transition-all text-gray-700 shadow-sm font-medium"
                                        ...
                                        @elseif ($question->type === 'geo')
                                            <div class="relative">
                                                ...
                                                <input type="text" name="{{ $inputName }}" placeholder="Enter location or coordinates" {{ $question->required ? 'required' : '' }}
                                                    class="w-full pl-12 pr-5 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#2271b1] transition-all text-gray-700 shadow-sm">
                                            </div>

                                        @elseif ($question->type === 'video')
                                            <div
                                                class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-xl bg-white hover:border-zinc-300 transition-colors">
                                                <div class="space-y-2 text-center">
                                                    <i class="fa-solid fa-video text-gray-500 text-3xl mb-2"></i>
                                                    <div class="flex text-sm text-gray-600 justify-center">
                                                        <label
                                                            class="relative cursor-pointer bg-white rounded-md font-bold text-[#2271b1] hover:text-zinc-2000 focus-within:outline-none focus-within:ring-2 focus-within:ring-[#2271b1] text-center">
                                                            <span>{{ __("Upload a video file") }}</span>
                                                            <input type="file" name="{{ $inputName }}" accept="video/*" class="sr-only" {{ $question->required ? 'required' : '' }}>
                                                        </label>
                                                    </div>
                                                    <p class="text-xs text-gray-600">{{ __("MP4, WebM, OGG up to 50MB") }}</p>
                                                </div>
                                            </div>
                                        ...
                                        @elseif ($question->type === 'audio')
                                            <div
                                                ...
                                                <div class="space-y-2 text-center">
                                                    ...
                                                    <p class="text-xs text-gray-600">{{ __("MP3, WAV, OGG up to 20MB") }}</p>
                                                </div>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <label class="flex items-start cursor-pointer group">
                                        <div class="flex items-center h-6">
                                            <input id="terms_and_conditions_legacy" name="terms_and_conditions" type="checkbox" required
                                                class="h-5 w-5 text-[#2271b1] border-gray-300 rounded focus:ring-[#2271b1] transition-all cursor-pointer shadow-sm">
                                        </div>
                                        <div class="ml-4 text-sm">
                                            <span
                                                class="font-bold text-gray-900 text-base group-hover:text-[#135e96] transition-colors">
                            ...
                            jQuery(function ($) {
                                ...
                                if (savedLegacyDraft) {
                                    try {
                                        ...
                                        if (hasRestored) {
                                            $('#legacySurveyForm').prepend('<div class="bg-zinc-100 text-[#135e96] p-3 rounded-lg text-sm font-medium mb-6 flex justify-between items-center shadow-sm border border-zinc-200" id="draftToast"><div class="flex items-center"><i class="fa-solid fa-cloud-arrow-down mr-2"></i> Your previous draft has been restored from local storage (Offline Draft Recovery Active).</div><button type="button" onclick="$(\'#draftToast\').fadeOut()" class="text-zinc-2000 hover:text-[#135e96]"><i class="fa-solid fa-xmark"></i></button></div>');
                                        }
                                    } catch (e) {
                                ...
                                // Submission proceeds directly to backend now
                            });
                        </script>
                    @endif
                </div>
...
<script>
    function startSurveyAsGuest() {
        const nameInput = document.getElementById('temp_guest_name');
        const phoneInput = document.getElementById('temp_guest_phone');
        ...
        if (!nameVal) {
            Swal.fire({
                ...
                text: 'Please enter your Name to proceed with the survey.',
                icon: 'warning',
        ...
        const jsonName = document.getElementById('guest_name_json');
        const jsonPhone = document.getElementById('guest_phone_json');
        if (jsonName) jsonName.value = nameVal;
        if (jsonPhone) jsonPhone.value = phoneVal;

        const legacyName = document.getElementById('guest_name_legacy');
        const legacyPhone = document.getElementById('guest_phone_legacy');
        if (legacyName) legacyName.value = nameVal;
        if (legacyPhone) legacyPhone.value = phoneVal;

        // Hide overlay and show content
        const overlay = document.getElementById('guest-details-overlay');
