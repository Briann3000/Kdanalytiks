        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden mb-6">
            <div class="p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 font-medium mt-1">{{ __('Manage your survey invitation campaigns, schedule emails, track response rates, and send reminders.') }}</p>
                </div>
                <div>
            ...
            @if($campaigns->isEmpty())
                <div class="p-16 flex flex-col items-center justify-center text-center">
                    ...
                    <p class="text-xs text-gray-400 font-medium mt-1 max-w-sm">{{ __('Start sending out invitations by creating your first campaign.') }}</p>
                </div>
            @else
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-50 bg-gray-50/50">
                                <th class="py-4 px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Campaign Name') }}</th>
                                <th class="py-4 px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Status') }}</th>
                                <th class="py-4 px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Recipients') }}</th>
                                <th class="py-4 px-6 text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Progress & Responses') }}</th>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group">
                                    ...
                                    <td class="py-4 px-6">
                                        @if($campaign->status === 'draft')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-gray-100 text-gray-500 border border-gray-200">
                                                {{ __('Draft') }}
                                            </span>
                                        @elseif($campaign->status === 'scheduled')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-amber-100 text-amber-600 border border-amber-200">
                                                {{ __('Scheduled') }}
                                            </span>
                                        ...
                                        @elseif($campaign->status === 'sending')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-indigo-100 text-indigo-600 border border-indigo-200">
                                                {{ __('Sending') }}
                                            </span>
                                        @elseif($campaign->status === 'completed')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-emerald-100 text-emerald-600 border border-emerald-200">
                                                {{ __('Completed') }}
                                            </span>
                                        @elseif($campaign->status === 'cancelled')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-rose-100 text-rose-600 border border-rose-200">
                                                {{ __('Cancelled') }}
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="w-48">
                                            <div class="flex items-center justify-between text-[9px] font-bold text-gray-500 mb-1">
                                                <span>{{ __('Responses: ') }} {{ $campaign->total_responded }} ({{ $campaign->recipients_count > 0 ? round(($campaign->total_responded / $campaign->recipients_count) * 100) : 0 }}%)</span>
                                            </div>
                                            ...
                                            <span class="block text-[8px] text-gray-400 font-medium mt-1">
                                                {{ __('Sent: ') }} {{ $campaign->total_sent }} | {{ __('Opened: ') }} {{ $campaign->total_opened }}
                                            </span>
                                        </div>
                                    </td>
