    ...
    <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden" x-data="{ 
                        ...
                        <div class="group relative bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 transition-all hover:shadow-2xl hover:border-indigo-200 hover:-translate-y-1 cursor-zoom-in"
                            ...
                            <div class="aspect-square flex items-center justify-center bg-gray-100/50 relative">
                                <template x-if="file.type === 'image'">
                                    <img :src="'/storage/' + file.path" class="w-full h-full object-cover"
                                        alt="Response Media">
                                </template>
                                ...
                                <!-- Badge -->
                                <div class="absolute top-2 right-2 px-1.5 py-0.5 bg-white/90 backdrop-blur-md border border-gray-100 rounded md text-[8px] font-black text-gray-900 uppercase tracking-tighter shadow-sm"
                                    x-text="file.type"></div>
                            </div>

                            <!-- Details -->
                            <div class="p-3 bg-white border-t border-gray-50">
                                <div class="text-[9px] font-bold text-gray-900 truncate"
                                    x-text="file.path.split('/').pop()"></div>
                                <div
                                    class="mt-1 flex items-center text-[8px] text-gray-400 font-bold uppercase tracking-widest">
        ...
        <div x-show="showLightbox" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="absolute top-8 right-8 flex gap-4">
                <a :href="activeMedia ? '/storage/' + activeMedia.path : '#'" download
                    class="w-12 h-12 bg-white/10 hover:bg-white/20 border border-white/10 rounded-2xl flex items-center justify-center text-white transition-all">
                    <i class="fa-solid fa-download text-xl"></i>
                </a>
                <button @click="showLightbox = false"
                    class="w-12 h-12 bg-white/10 hover:bg-rose-600 border border-white/10 rounded-2xl flex items-center justify-center text-white transition-all">
            ...
            <div class="w-full h-full flex items-center justify-center" @click.self="showLightbox = false">
                <template x-if="activeMedia && activeMedia.type === 'image'">
                    <img :src="'/storage/' + activeMedia.path"
                        class="max-w-full max-h-full rounded-2xl shadow-2xl border-4 border-white/10" alt="Preview">
                </template>
                <template x-if="activeMedia && activeMedia.type === 'video'">
                    <video controls class="max-w-full max-h-full rounded-2xl shadow-2xl border-4 border-white/10"
                        :src="'/storage/' + activeMedia.path"></video>
                </template>
