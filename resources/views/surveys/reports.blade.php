...
@section('survey-content')
    ...
    @php
        $allowedReportTabs = ['quantitative', 'qualitative', 'inferential', 'crosstab', 'analyse', 'humanizer', 'compiled'];
    ...
    <div :class="(reportTab === 'analyse' || reportTab === 'humanizer') ? '' : 'space-y-12'" x-data="{
        ...
        <div x-show="reportTab !== 'analyse'"
            ...
            <div class="flex flex-wrap items-center gap-6">
                ...
                <!-- Unified Group -->
                <div class="flex flex-col gap-1.5">
                    <span class="text-xs font-bold text-indigo-600 tracking-tight pl-1">{{ __('Full Synthesis') }}</span>
                    <div class="flex items-center gap-1 bg-indigo-50/60 p-1 rounded-2xl w-fit">
                        <button @click="switchReportTab('compiled')"
                            :class="reportTab === 'compiled' ? 'bg-indigo-600 text-white shadow-sm' : 'text-indigo-700 hover:text-indigo-900'"
        ...
        <div x-show="reportTab === 'quantitative' && isPremium"
            class="bg-gradient-to-r from-indigo-50 to-indigo-50/20 rounded-3xl p-6 border border-indigo-100/80 shadow-inner mb-10 flex flex-col md:flex-row items-center justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-300"
            style="display: none;">
            <div class="flex items-center gap-4 w-full md:w-auto shrink-0">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-gray-500 font-bold"
                        x-text="globalRefining ? globalRefineProgress : '{{ __('Review the entire report') }}'"></p>
                </div>
            </div>

            <div class="flex flex-1 items-center gap-2 sm:gap-3 w-full relative">
                <textarea x-model="globalFeedback" :disabled="globalRefining"
                    placeholder="{{ __('Reflect your own voice...') }}"
                    @keydown.enter.prevent="if(!$event.shiftKey && !$event.ctrlKey) globalRefineAll()" rows="2"
                    class="w-full bg-white border border-indigo-100 text-xs font-semibold rounded-xl sm:rounded-2xl px-3 sm:px-5 py-2.5 sm:py-3 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all disabled:bg-gray-50 disabled:text-gray-400 custom-scrollbar resize-none"></textarea>

                <button @click="globalRefineAll()" :disabled="globalRefining || !globalFeedback.trim()"
                    class="px-3 sm:px-6 py-3 sm:py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl sm:rounded-2xl text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-1 sm:gap-2 whitespace-nowrap self-stretch">
                    <i x-show="globalRefining" class="fa-solid fa-circle-notch fa-spin text-xs"></i>
                    <span class="hidden sm:inline"
                        x-text="globalRefining ? '{{ __('Refining...') }}' : '{{ __('Apply to All Questions') }}'"></span>
                    <span class="sm:hidden"
                        x-text="globalRefining ? '{{ __('Refining...') }}' : '{{ __('Apply') }}'"></span>
                </button>
            </div>
        </div>
                                        ...
                                        @else
                                            ...
                                            <tbody class="divide-y divide-gray-200">
                                                ...
                                                @foreach($item['stats'] as $stat)
                                                    ...
                                                    <tr class="hover:bg-gray-50/30 transition-colors border-b border-gray-100">
                                                        <td
                                                            class="py-2 sm:py-2.5 px-2 sm:px-4 text-[10px] sm:text-xs font-normal text-gray-700 break-words leading-tight border-r border-gray-200">
                                                            {{ $stat['value'] }}
                                                        </td>
                                                        <td
                                                            class="py-2 sm:py-2.5 px-2 sm:px-4 text-right text-[10px] sm:text-xs font-normal text-gray-900 border-r border-gray-200">
                                                            {{ number_format($stat['count']) }}
                                                        </td>
                                                        <td
                                                            class="py-2 sm:py-2.5 px-2 sm:px-4 text-right text-[10px] sm:text-xs font-normal text-gray-900 border-r border-gray-200">
                                            ...
                                            <tfoot class="bg-gray-50/50 border-t-2 border-gray-200">
                                                <tr class="font-normal text-gray-900 text-[10px] sm:text-[11px]">
                                                    <td class="py-2 sm:py-3 px-2 sm:px-4 tracking-wider border-r border-gray-200">
                                                        {{ __('Total') }}
                                                    </td>
                                                    <td class="py-2 sm:py-3 px-2 sm:px-4 text-right border-r border-gray-200">
                                                        {{ number_format($totalFreq) }}
                                                    </td>
                                                    <td class="py-2 sm:py-3 px-2 sm:px-4 text-right border-r border-gray-200">100.0%
                                                    </td>
                                                    <td class="py-2 sm:py-3 px-2 sm:px-4 text-right border-r border-gray-200">100.0%
                                                    </td>
                            ...
                            <div class="px-8 py-6 border-b border-gray-100 bg-white flex justify-between items-center">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex gap-2" data-html2canvas-ignore>
                                        ...
                                        <button
                                            onclick="window.exportTableToCsv('qual-table-{{ $loop->index }}', '{{ addslashes($item['label']) }}')"
                                        ...
                                        <button
                                            onclick="window.exportTableToPng('qual-wrapper-{{ $loop->index }}', '{{ addslashes($item['label']) }}')"
        ...
        <!-- Compiled Unified Report Content -->
        <div x-show="reportTab === 'compiled'" class="space-y-8 animate-in fade-in duration-500" style="display: none;">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-5">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">{{ __('Compiled Survey Report') }}</h2>
                    <p class="text-xs text-gray-500 font-medium">
                        {{ __('Unified sequential analysis containing quantitative, qualitative and inferential findings.') }}
                    </p>
                </div>
                <div class="flex items-center gap-3 shrink-0">
                    <a href="{{ route('surveys.export_compiled_docx', $survey) }}"
                        class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-sm transition-all flex items-center gap-2">
                        <i class="fa-solid fa-file-word"></i>
                        <span>{{ __('Export DOCX') }}</span>
                    </a>
                    <a href="{{ route('surveys.export_compiled_pdf', $survey) }}"
                        class="px-4 py-2 bg-zinc-800 hover:bg-zinc-900 text-white rounded-xl text-xs font-bold shadow-sm transition-all flex items-center gap-2">
            ...
            <!-- Sequential Survey Questions -->
            @foreach($analysis as $item)
                @if($item['isChartable'])
                    <!-- Quantitative Question Sequential Container -->
                    <div class="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm space-y-6">
                        <div class="border-b border-gray-100 pb-6">
                            <h4 class="text-base sm:text-lg font-black text-gray-900 leading-snug">
                                <span class="text-indigo-600 mr-2 opacity-30 text-base font-black">#{{ $loop->iteration }}</span>
                                {{ $item['label'] ?? $item['question_text'] ?? $item['question'] ?? ('Question #' . $loop->iteration) }}
                            </h4>
                        </div>
                        ...
                        <div class="flex flex-col gap-10">
                            @if(empty($item['isLikertLike']))
                                ...
                                <div
                                    class="h-96 w-full max-w-4xl mx-auto relative flex items-center justify-center bg-gray-50/30 rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-inner group/chart">
                                    <canvas id="compiled-{{ $item['canvasId'] }}"></canvas>
                                </div>
                            @endif

                            <!-- Table Area (Below Chart) -->
                            <div class="bg-white rounded-2xl border border-gray-100 relative z-20"
                                id="compiled-table-wrapper-{{ $item['canvasId'] }}">
                                <div
                                    class="px-6 py-4 bg-gray-50/50 border-b border-gray-100 flex justify-between items-center rounded-t-2xl">
                                    <span class="text-xs font-bold text-zinc-500 tracking-tight">
                                        {{ empty($item['isLikertLike']) ? __('Frequency Table') : __('Likert Matrix Table') }}
                                    </span>
                                </div>
                                <div class="w-full rounded-b-2xl overflow-x-auto">
                                    <table class="w-full text-left border border-gray-200"
                                        id="compiled-table-{{ $item['canvasId'] }}">
                                        @if(!empty($item['isLikertLike']))
                                            <thead class="bg-gray-50/70 border-b border-gray-200 shadow-sm">
                                                <tr class="text-[9px] sm:text-[10px] font-bold text-gray-500 tracking-wider">
                                                    <th class="py-2 px-3 border-r border-gray-200 bg-gray-50/90 w-1/4 sm:w-1/3 row-span-2 text-left"
                                                        rowspan="2">
                                                        {{ __('Item') }}
                                                    </th>
                                                    @foreach($item['stats'] as $stat)
                                                        @if(!isset($stat['is_missing']) || !$stat['is_missing'])
                                                            <th colspan="2"
                                                                class="hidden sm:table-cell py-1.5 px-3 text-center border-b border-r border-gray-200 bg-gray-50/50 whitespace-nowrap font-bold text-gray-800">
                                                                {{ $stat['value'] }}
                                                            </th>
                                                        @endif
                                                    @endforeach
                        ...
                        <!-- Quantitative AI interpretation card exactly as in quantitative -->
                        <x-ai-quant-insight-card :question-id="$item['id']" :survey-id="$survey->id" :stats="$item['stats']"
                            :read-only="true" />
                    </div>
                @else
                    <!-- Qualitative Question Sequential Container -->
                    <div class="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm space-y-6">
                        <div class="border-b border-gray-100 pb-6">
                            <h4 class="text-base sm:text-lg font-black text-gray-900 leading-snug">
                                <span class="text-indigo-600 mr-2 opacity-30 text-base font-black">#{{ $loop->iteration }}</span>
                        ...
                        @if($item['isAnalyzable'])
                            <x-ai-insight-card :question-id="$item['id']" :question-title="$item['label']"
                                :survey-id="$item['survey_id']" :index="$loop->index" :read-only="true" />
                        @endif
                    </div>
                @endif
            @endforeach

            <!-- Section 3: Saved Inferential Analyses -->
            <div class="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm space-y-6">
                <div class="flex items-center justify-between border-b border-gray-100 pb-4">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-10 h-10 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-600 font-bold">
                            <i class="fa-solid fa-calculator text-sm"></i>
                        </div>
                        <div>
                            <h3 class="text-base font-black text-gray-900">{{ __('Inferential Statistical Analyses') }}</h3>
                            <p class="text-xs text-gray-500 font-medium">
                                {{ __('Saved statistical significance and hypothesis tests.') }}
                            </p>
                        </div>
                    </div>
                    <span class="px-3 py-1 bg-purple-50 text-purple-700 rounded-full text-xs font-bold">
                        {{ $savedInferentialTests->count() }} {{ __('Saved Tests') }}
                    </span>
                </div>
                ...
                @if($savedInferentialTests->count() > 0)
                    <div class="space-y-6">
                        @foreach($savedInferentialTests as $test)
                            <div class="p-6 bg-purple-50/20 rounded-2xl border border-purple-100/60 space-y-4">
                                <div class="flex justify-between items-center border-b border-purple-100/40 pb-3">
                                    <h4 class="text-sm font-black text-purple-900 uppercase tracking-wider">
                                        {{ $test->test_name ?? $test->test_type }}
                                    </h4>
                                    <span
                                        class="text-xs text-gray-400 font-semibold">{{ $test->created_at->format('M d, Y') }}</span>
                                </div>
                                <div class="text-xs sm:text-sm text-gray-700 leading-relaxed font-normal whitespace-pre-line">
                                    {{ str_replace('**', '', $test->ai_interpretation ?? $test->summary ?? '') }}
                                </div>
                            </div>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-12 bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
                        <i class="fa-solid fa-flask text-gray-300 text-3xl mb-3"></i>
                        <p class="text-xs font-bold text-gray-500">
                            {{ __('No inferential tests have been saved for this survey yet.') }}
                        </p>
                        <p class="text-[11px] text-gray-400 mt-1.5">
                            {{ __('Run and save Chi-Square, t-Tests, or Regression under the "Analyze" tab to include them here.') }}
                        </p>
                    </div>
                @endif
            </div>
            ...
            <div x-data="inferentialManager({{ $savedInferentialTests->toJson() }})" class="space-y-6">
                ...
                <div class="flex flex-col lg:flex-row gap-6 items-start">
                    ...
                    <div class="w-full" :class="sidebarOpen ? 'lg:w-[calc(100%-17rem)]' : 'w-full'">
                        <div
                            class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm relative overflow-hidden space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-50 pt-6">
                                ...
                                <template x-if="testMethod === 'crosstab' || testMethod === 'chisquare'">
                                    <div class="contents">
                                        <div>
                                            <label
                                                class="block text-[12px] font-medium text-gray-500  tracking-widest mb-2">{{ __('Row Variable ') }}</label>
                                            <select x-model="rowVar"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all max-w-full">
                                        ...
                                        <div>
                                            <label
                                                class="block text-[12px] font-medium text-gray-500  tracking-widest mb-2">{{ __('Column Variable ') }}</label>
                                ...
                                <template x-if="testMethod === 'cronbach'">
                                    <div class="col-span-full space-y-4">
                                        <label
                                            class="block text-xs font-bold text-gray-700  tracking-wider">{{ __('Select Likert / Rating Scale Items for Reliability Testing') }}</label>
                                ...
                                <template x-if="testMethod === 'ttest' || testMethod === 'anova'">
                                    <div class="contents">
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Dependent Variable (Numeric)') }}</label>
                                            <select x-model="depVar"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all">
                                                <option value="">{{ __('Select Question...') }}</option>
                                                @foreach($analysis as $item)
                                                    @if($item['isChartable'])
                                                        <option value="{{ $item['id'] }}">
                                        ...
                                        <div>
                                            <label
                                                class="block text-[12px] font-medium text-gray-500  tracking-widest mb-2">{{ __('Grouping Variable ') }}</label>
                                ...
                                <template x-if="testMethod === 'correlation'">
                                    <div class="contents">
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Variable X (Numeric)') }}</label>
                                            <select x-model="varX"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all">
                                                <option value="">{{ __('Select Question...') }}</option>
                                                @foreach($analysis as $item)
                                        ...
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Variable Y (Numeric)') }}</label>
                                            <select x-model="varY"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all">
                                                <option value="">{{ __('Select Question...') }}</option>
                                                @foreach($analysis as $item)
                                ...
                                <template x-if="testMethod === 'regression'">
                                    <div class="contents">
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Dependent Variable Y (Numeric)') }}</label>
                                            <select x-model="depVar"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all">
                                                <option value="">{{ __('Select Question...') }}</option>
                                                @foreach($analysis as $item)
                                        ...
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Independent Variable X (Numeric)') }}</label>
                                            <select x-model="groupVar"
                                                class="w-full bg-gray-50 border border-gray-200 text-sm font-medium rounded-xl px-4 py-3.5 focus:ring-indigo-500 transition-all">
                                                <option value="">{{ __('Select Question...') }}</option>
                                                @foreach($analysis as $item)
                                ...
                                <template x-if="testMethod === 'regression_multiple'">
                                    <div class="contents">
                                        ...
                                        <div>
                                            <label
                                                class="block text-[10px] font-black text-gray-500  tracking-widest mb-2">{{ __('Independent Variables X (Select Multiple)') }}</label>
                                            <div
                        ...
                        <template
                            ...
                            <div class="mt-8 pt-8 border-t border-gray-100 flex flex-col items-center">
                                <div x-show="aiLoading" class="flex items-center gap-3 text-indigo-600 py-4">
                                    ...
                                    <span
                                        class="text-xs font-black">{{ __('Analyzing Statistical Significance...') }}</span>
                                </div>

                                <div x-show="aiMessages.length > 0"
                                    class="w-full bg-gradient-to-br from-indigo-50 to-white rounded-3xl p-8 border border-indigo-100 shadow-inner relative overflow-hidden">
                                    <i
                                        class="fa-solid fa-brain absolute right-[-20px] top-[-20px] text-[120px] text-indigo-600/5"></i>
                                    <div class="relative z-10 flex flex-col md:flex-row gap-6 items-start">
                                        <div
                                            class="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white shrink-0 shadow-lg shadow-indigo-200">
                                            <i class="fa-solid fa-comments text-lg"></i>
                                        </div>
                                        <div class="w-full">
                                            <h5 class="text-sm font-black text-indigo-600 mb-3">
                                                {{ __('Statistical Intelligence') }}
                                            </h5>
                                            ...
                                            {{--
                                            <div x-show="!aiPolishing"
                                                class="border-t border-indigo-100/50 pt-4 mt-4 w-full">
                                                <div class="flex flex-col md:flex-row gap-3 items-end">
                                                    <div class="flex-1 w-full">
                                                        <label class="block text-xs font-bold text-indigo-600 mb-1.5">{{
                                                            __('Refine this statistical interpretation') }}</label>
                                                        <input x-model="aiFeedback" type="text"
                                                            placeholder="{{ __('Reflect your own voice...') }}"
                                                            @keydown.enter="polishAiInsight()"
                                                            class="w-full bg-white/70 border border-indigo-100 text-xs font-semibold rounded-xl px-3 py-2.5 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all">
                                                    </div>
                                                    <button @click="polishAiInsight()"
                                                        :disabled="aiPolishing || !aiFeedback.trim()"
                                                        class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-1.5 self-stretch justify-center whitespace-nowrap">

                                                        <span
                                                            x-text="aiPolishing ? '{{ __('Polishing...') }}' : '{{ __('Polish') }}'"></span>
                                                    </button>
                                                </div>
                                            </div>
                                            --}}
                                            ...
                                            <div x-show="aiMessages.length > 0 && !aiLoading"
                                                class="mt-4 pt-3 border-t border-indigo-100/30 flex items-center justify-end"
                                                style="display: none;">
                                                <button @click="aiMessages = []; aiInsight = null; aiFeedback = '';"
                                ...
                                <button x-show="!aiLoading && aiMessages.length === 0" @click="getAiInterpretation()"
                                    ...
                                    <i class="fa-solid fa-brain group-hover:scale-110 transition-transform"></i>
                                    {{ __('Interpret with AI') }}
                                </button>
                            </div>
                        </template>
                    </div>
                            ...
                            return {
                                ...
                                init() {
                                    ...
                                    chartConfigs.forEach(config => {
                                        ...
                                        const elComp = document.getElementById('compiled-' + config.canvas_id);
                                        if (elComp) {
                                            const defaultType = savedTypes[config.canvas_id] || 'bar';
                                            const defaultColor = savedColors[config.canvas_id] || 'vibrant';
                                            this.chartTypes['compiled-' + config.canvas_id] = defaultType;
                                            this.activeColors['compiled-' + config.canvas_id] = defaultColor;
                                            chartInstances['compiled-' + config.canvas_id] = createChart('compiled-' + config.canvas_id, config, defaultType, defaultColor);
                                        }
                                    });
                                },
                                switchChartType(canvasId, type) {
                                    this.chartTypes[canvasId] = type;
                                    if (!canvasId.startsWith('compiled-')) {
                                ...
                                refreshChart(canvasId) {
                                    const baseCanvasId = canvasId.replace('compiled-', '');
                                    const config = chartConfigs.find(c => c.canvas_id === baseCanvasId);
                                    if (chartInstances[canvasId]) {
                                        chartInstances[canvasId].destroy();
                                        delete chartInstances[canvasId];
                                    }
                ...
                check() {
                    ...
                    // Show top button if scrolled down more than 50px
                    this.showTop = scrollTop > 50; 

                    // Show bottom button if there is MORE than 100px of scrollable content below current position
                    this.showBottom = (scrollTop + clientHeight) < (scrollHeight - 100);
                }
            }" x-init="$nextTick(() => check())" @scroll.window.throttle.50ms="check()"
                    class="fixed bottom-12 right-6 z-[9999] flex flex-col gap-2">

                    <!-- TOP BUTTON -->
                    <button x-show="showTop" x-transition @click="window.scrollTo({top: 0, behavior: 'smooth'})"
                        class="w-12 h-12 bg-[#2271b1] hover:bg-[#135e96] text-white rounded-full shadow-2xl flex items-center justify-center transition-all cursor-pointer border-2 border-white">
