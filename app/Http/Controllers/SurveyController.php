...
class SurveyController extends Controller
{
    ...
    public function exportJson(\App\Models\Survey $survey)
    {
        ...
        $output = [
            'generated_by' => $branding['showKdBranding'] ? 'KDAnalytics' : ($branding['customOrgName'] ?? 'Export System'),
            'survey_id' => $survey->id,
            'data' => $data
        ];
    ...
    public function exportXml(\App\Models\Survey $survey)
    {
        ...
        if ($branding['showKdBranding']) {
            $xml->writeComment(' Generated by KDAnalytics | kmsurveytool.com ');
    ...
    public function exportSpss(\App\Models\Survey $survey)
    {
        ...
        $writer = new \SPSS\Sav\Writer([
            'header' => [
                'prodName' => $branding['showKdBranding'] ? 'KDAnalytics' : ($branding['customOrgName'] ?? ''),
    ...
    public function exportGoogleSheets(\App\Models\Survey $survey)
    {
        ...
        $branding = $this->getBrandingContext($survey);
        $titleSuffix = $branding['showKdBranding'] ? ' (via KDAnalytics)' : ($branding['customOrgName'] ? ' (for ' . $branding['customOrgName'] . ')' : '');
    ...
    public function exportResponses(\App\Models\Survey $survey)
    {
        ...
        if ($branding['showKdBranding']) {
            fputcsv($handle, ['# Exported via KDAnalytics — kmsurveytool.com']);
        } elseif ($branding['customOrgName']) {
