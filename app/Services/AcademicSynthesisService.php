    ...
    public function exportToDocx(array $content, string $filename, array $branding = [])
    {
        ...
        if (!empty($branding)) {
            if ($branding['showKdBranding']) {
                ...
                $footer->addPreserveText('Generated by KDAnalytics - Page {PAGE} of {NUMPAGES}', ['bold' => true, 'size' => 10, 'color' => '999999'], ['alignment' => \PhpOffice\PhpWord\SimpleType\Jc::CENTER]);
            } else {
    ...
    public function exportToPdf(array $content, string $filename, array $branding = [])
    {
        ...
        if (!empty($branding)) {
            if ($branding['showKdBranding']) {
                ...
                $mpdf->SetWatermarkText('KDAnalytics');
